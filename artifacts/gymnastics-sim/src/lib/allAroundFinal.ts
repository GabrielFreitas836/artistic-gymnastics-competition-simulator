...
import { RankedGymnast, getAllAroundRankings } from "./rankings";
import { getApparatusComponents } from "./scoring";
import {
  AllAroundFinalSlot,
  ApparatusKey,
  DnsMap,
...
const getAllGymnasts = (state: SimulationState): Gymnast[] => [
  ...Object.values(state.teams).flatMap((team) => team.gymnasts),
  ...Object.values(state.mixedGroups).flatMap((group) => group.gymnasts),
];

const getGymnastLookup = (state: SimulationState): Map<string, Gymnast> =>
  new Map(getAllGymnasts(state).map((gymnast) => [gymnast.id, gymnast]));

export const getAllAroundFinalQualificationPool = (
  state: SimulationState,
): AllAroundFinalQualificationPool => {
  const rankings = getAllAroundRankings(getAllGymnasts(state), state.scores, state.dns);
...
export const getAllAroundFinalRankings = (
  ...
  const gymnastLookup = getGymnastLookup(state);
  ...
  const mappedRows: Array<AllAroundFinalRankingRow | null> = slots.map((slot) => {
      ...
      return {
        ...
        isDnf: ALL_AROUND_FINAL_APPARATUS.some(
          (apparatusKey) => apparatus[apparatusKey].resultState === "DNF",
        ),
